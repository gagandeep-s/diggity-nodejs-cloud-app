...
console.log("ASHISH Request Data:", {
    typeOfTokens: typeof (req.tokens),
    tokens: req.tokens,
    title : req.title,
    body : req.body,
    typeOfData: typeof (req.data),
    data: req.data
});
/*
        if (req && req.tokens && req.title && req.body) {
            let payload = {
                notification: {
                    title: req.title, //iOS, Android, Web: The notification's title.
                    body: req.body, //iOS, Android, Web: The notification's body text.
                    // badge?: string, //???
                    // clickAction: string, //???
                    // color: string, //???
                    // icon: string, //???
            ...
            if (req.data && Object.keys(req.data).length > 0) {
                payload.data = req.data; //The keys and values must both be strings. Keys can be any custom string, except for the following reserved strings: "from" & Anything starting with "google."
            }

            admin.messaging().sendToDevice(req.tokens, payload, {
